...
public class AdminMenuController {
    ...
    private void guardarMenu() {
        // Lógica para guardar los cambios en el menú
        try {
            // Ejemplo: guardar en un archivo
            Files.createDirectories(Paths.get("DataBase/menus"));
            try (FileWriter writer = new FileWriter("DataBase/menus/menu_semanal.txt")) {
                writer.write("Menú actualizado por: " + usuarioActual + "\n");
                // Aquí irían los datos del menú
            }
            JOptionPane.showMessageDialog(view, "Menú guardado exitosamente", "Éxito", JOptionPane.INFORMATION_MESSAGE);
        } catch (IOException e) {
            JOptionPane.showMessageDialog(view, "Error al guardar el menú: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void volverAOpUsuario() {
        ...
        SwingUtilities.invokeLater(() -> {
            Op_Usuario opUsuario = new Op_Usuario(usuarioActual, true); // true porque solo admin llega aquí
